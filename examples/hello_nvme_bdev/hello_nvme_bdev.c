/*************************************************************************
  ...
  > Description:
    ...
    Similar to hello_bdev.c provided in SPDK, this program demos how to
    create a block device and write a hello world message into it and read
    the message from it. However, unlike hello_bdev.c, we only support NVMe
    physical device (hello_bdev.c provides a unified interface that can support
    various underlying device. It is done by reading in a configuration file.
    However, doing so requires us to create block device at first through
    a separate program. That's not what we want)
 ************************************************************************/
