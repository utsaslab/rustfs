...
impl FS{
    ...
    pub fn alloc_block(&mut self) -> usize{
        ...
        let zero_buf = spdk_rs::env::dma_zmalloc(&self.device.blk_size 
                                as usize, &self.device.buf_align);
        &mut self.device.write(&mut zero_buf, offset, &self.device.blk_size);
    ...
    pub fn mkfs(&mut self){
        let zero_buf = spdk_rs::env::dma_zmalloc(&self.device.blk_size 
                                as usize, &self.device.buf_align);
        let &mut write_buf = spdk_rs::env::dma_zmalloc(&self.device.blk_size
                                as usize, &self.device.buf_align);
        ...
        &mut self.device.write(&write_buf, &self.device.blk_size, &self.device.blk_size);
        &mut self.device.write(&zero_buf, 2*&self.device.blk_size, &self.device.blk_size);
        root = Option<Inode::new(&mut self, DIR_TYPE, 0)>;
        write_buf.fill(blk_size as usize, "%s\n", root.to_string());
        &mut self.device.write(&write_buf, 3*&self.device.blk_size, &self.device.blk_size);

    }

    pub fn mount(&mut self){
        let &mut read_buf = spdk_rs::env::dma_zmalloc(&self.device.blk_size
                                as usize, &self.device.buf_align);
        &mut self.device.read(&read_buf, &self.device.blk_size, &self.device.blk_size);
        inode_bitmap.bitmap = read_buf.read();
        &mut self.device.read(&read_buf, 2*&self.device.blk_size, &self.device.blk_size);
        data_bitmap.bitmap = read_buf.read();
        &mut self.device.read(&read_buf, 3*&self.device.blk_size, &self.device.blk_size);
